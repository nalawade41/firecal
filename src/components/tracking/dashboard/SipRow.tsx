...
import { SIP_BADGE_VARIANT, SIP_STATUS_LABEL } from "./constants/Dashboard.constants"
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-white/5 last:border-0">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-[13px] font-medium text-white/80 font-['DM_Mono',monospace]">{sip.amount}</span>
        <StatusBadge variant={SIP_BADGE_VARIANT[sip.status]}>{statusLabel}</StatusBadge>
      </div>
    </div>
  )
}
