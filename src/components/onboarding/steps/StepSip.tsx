...
function newSip(): SipEntry {
  return { id: `sip-${Date.now()}`, fundName: "", amount: 0, startDate: "", unitsTillNow: 0, goalId: "", folioNumber: "" }
}
      ...
      {entries.map((entry, idx) => (
        <div key={entry.id} className="space-y-3 p-4 rounded-lg border border-slate-200 bg-slate-50/50">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-slate-800">SIP {idx + 1}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <NumberField
              ...
              onChange={(v) => updateEntry(idx, { unitsTillNow: v })}
              step={0.001}
            />
